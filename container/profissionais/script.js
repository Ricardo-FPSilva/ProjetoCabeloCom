...
function editarProfissional(index) {
  ...
  const novoStatus = prompt(
    "Novo status (Livre, Disponível, Indisponível):",
  ...
  if (profissionais.length === 0) {
    ...
    profissionais.forEach((prof, i) => {
      ...
      card.innerHTML = `
                        ...
                        <div class="detalhes">
                        <div class="info">📊 Status: <span class="status ${prof.status.toLowerCase()}">${
        prof.status
      }</span></div>
                        <div class="info">💰 Valor no dia: R$ <span class="valor-dia">${
                          prof.valor
                        }</span></div>
                        <div class="info">💳 Vales solicitados: R$ <span class="vales-dia">${
                          prof.vales
                        }</span></div>
                        <div class="info">
